...
$("sendAsset").onchange =
	() => {
		...
		$("sendAmountLabel").textContent =
			isBtc ?
			"Quantidade de sats" :
			"Quantidade de DePix";

		$("sendAmount").placeholder =
			isBtc ?
			"ex: 5000" :
			"ex: 1.5";
		...
		try {
            ...
            14
              ...
              14
					...
					appendLog(
						lg,
						`Enviando ${amount} sats (Lightning) para ${to.slice(
              0,
              24
            )}…`
					);
...
$("btnExitQuote").onclick =
	async () => {
		...
		try {
			appendLog(
				lg,
				"Consultando taxa de exit…"
			);

			const quote =
				await getExitFeeQuote(
					amount,
					address
				);
